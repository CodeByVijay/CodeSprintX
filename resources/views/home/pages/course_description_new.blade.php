...
@section('content')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div>
                        ...
                        <!-- Tab Navigation -->
                        <div class="mb-6">
                            <div class="flex border-b border-gray-200" role="tablist" aria-label="Program Duration Tabs">
                                <button id="tab-3-month" class="program-tab px-6 py-3 text-lg font-medium text-blue-600 border-b-2 border-blue-600 bg-blue-50" role="tab" aria-selected="true" aria-controls="content-3-month" tabindex="0">
                                    3 Month Program
                                </button>
                                <button id="tab-6-month" class="program-tab px-6 py-3 text-lg font-medium text-gray-500 hover:text-gray-700" role="tab" aria-selected="false" aria-controls="content-6-month" tabindex="-1">
                                    6 Month Program
                                </button>
                            </div>
                        </div>
                        ...
                        <div id="content-3-month" class="program-content">
                            <div class="border border-gray-200 rounded-lg overflow-hidden" role="tabpanel" aria-labelledby="tab-3-month">
                                <!-- Week 1-2 -->
                        ...
                        <div id="content-6-month" class="program-content hidden">
                            <div class="border border-gray-200 rounded-lg overflow-hidden" role="tabpanel" aria-labelledby="tab-6-month">
                                <!-- Month 1 -->
                                <div class="border-b border-gray-200 last:border-b-0">
                                    <div class="bg-gray-50 p-4">
    ...
    <div id="paymentModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
            ...
            <h3 class="text-2xl font-bold mb-4 text-center">Confirm Your Purchase</h3>
            <div class="mb-4">
                ...
                <div class="flex justify-between mb-2">
                    <span class="font-medium">Processing Fees (2%):</span>
                    <span id="modalProcessing"></span>
                </div>
                <div class="flex justify-between mt-4 border-t pt-2">
                    <span class="font-bold">Subtotal:</span>
                    <span id="modalSubtotal" class="font-bold text-primary"></span>
                </div>
            </div>
            <button id="processPaymentBtn" class="w-full py-3 bg-primary text-white rounded-md font-bold hover:bg-secondary transition-colors">Process &amp; Pay</button>
        </div>
    </div>
        ...
        <div class="container mx-auto px-4 text-center">
            ...
            <div class="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4">
                <button
                    ...
                    data-course="{{ $currentCourse['title'] }}" data-token="{{ $currentCourse['id'] }}"
                    data-duration="{{ Crypt::encrypt('3') }}"
                    data-fees="@if(isset($currentCourse['original_price_3_month']) && isset($currentCourse['offer_end_at']) && strtotime($currentCourse['offer_end_at']) > time()){{ $currentCourse['price_3_month'] }}@elseif(isset($currentCourse['original_price_3_month'])){{ $currentCourse['original_price_3_month'] }}@else{{ $currentCourse['price_3_month'] }}@endif"
                    data-months="3">
                    <span>
                        Buy 3-Month Plan
                        @if (isset($currentCourse['original_price_3_month']) && isset($currentCourse['offer_end_at']) && strtotime($currentCourse['offer_end_at']) > time())
                ...
                <button
                    ...
                    data-course="{{ $currentCourse['title'] }}" data-token="{{ $currentCourse['id'] }}"
                    data-duration="{{ Crypt::encrypt('6') }}"
                    data-fees="@if(isset($currentCourse['original_price_6_month']) && isset($currentCourse['offer_end_at']) && strtotime($currentCourse['offer_end_at']) > time()){{ $currentCourse['price_6_month'] }}@elseif(isset($currentCourse['original_price_6_month'])){{ $currentCourse['original_price_6_month'] }}@else{{ $currentCourse['price_6_month'] }}@endif"
                    data-months="6">
                    <span>
                        Buy 6-Month Plan
                        @if (isset($currentCourse['original_price_6_month']) && isset($currentCourse['offer_end_at']) && strtotime($currentCourse['offer_end_at']) > time())
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Open modal on any buy button click
            document.querySelectorAll('.razorpay-buy-btn, .cta-buy-btn').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    // Only open modal, do not trigger payment directly
                    const course = btn.getAttribute('data-course');
                    const duration = btn.getAttribute('data-months') === '6' ? '6 Months' : '3 Months';
                    let fees = btn.getAttribute('data-fees');
                    if (!fees || isNaN(parseFloat(fees))) {
                        // fallback: try to get from price_3_month/price_6_month
                        if (btn.getAttribute('data-months') === '6') {
                            fees = '{{ isset($currentCourse['price_6_month']) ? $currentCourse['price_6_month'] : (isset($currentCourse['original_price_6_month']) ? $currentCourse['original_price_6_month'] : 0) }}';
                        } else {
                            fees = '{{ isset($currentCourse['price_3_month']) ? $currentCourse['price_3_month'] : (isset($currentCourse['original_price_3_month']) ? $currentCourse['original_price_3_month'] : 0) }}';
                        }
                    }
                    fees = parseFloat((fees+'').replace(/[^\d.]/g, ''));
                    const token = btn.getAttribute('data-token');
                    const encDuration = btn.getAttribute('data-duration');

                    // Calculate GST and Processing
                    const gst = Math.round(fees * 0.18 * 100) / 100;
                    const processing = Math.round(fees * 0.02 * 100) / 100;
                    const subtotal = Math.round((fees + gst + processing) * 100) / 100;

                    document.getElementById('modalCourseName').textContent = course;
                    document.getElementById('modalDuration').textContent = duration;
                    document.getElementById('modalFees').textContent = formatINR(fees);
                    document.getElementById('modalGST').textContent = formatINR(gst);
                    document.getElementById('modalProcessing').textContent = formatINR(processing);
                    document.getElementById('modalSubtotal').textContent = formatINR(subtotal);

                    // Store for payment
                    document.getElementById('processPaymentBtn').dataset.fees = fees;
                    document.getElementById('processPaymentBtn').dataset.gst = gst;
                    document.getElementById('processPaymentBtn').dataset.processing = processing;
                    document.getElementById('processPaymentBtn').dataset.subtotal = subtotal;
                    document.getElementById('processPaymentBtn').dataset.course = course;
                    document.getElementById('processPaymentBtn').dataset.token = token;
                    document.getElementById('processPaymentBtn').dataset.duration = encDuration;
            ...
            document.getElementById('processPaymentBtn').addEventListener('click', function() {
                // You should POST these values to your backend for Razorpay order creation
                // and then open Razorpay checkout with the returned order_id and amount
                // This is a placeholder for demo only
                const subtotal = this.dataset.subtotal;
                const course = this.dataset.course;
                const token = this.dataset.token;
                const duration = this.dataset.duration;

                // Example: AJAX to backend (replace URL and logic as needed)
                    ...
                    if(data.razorpay_order_id && data.amount){
                        ...
                        var options = {
                            ...
                            handler: function (response){
                                ...
                                window.location.href = '/payment/success?order_id=' + data.razorpay_order_id;
                            },
